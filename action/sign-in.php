<?php
    ...
    if ($row > 0) {
        ...
        $sql_log = "SELECT * FROM log_member WHERE m_id = '$m_id ' ORDER BY lm_date_in DESC";
        ...
        $sql = "INSERT INTO `log_employee`( `e_id`, `le_date_in`) VALUES ('$e_id',NOW())";
        $result = mysqli_query($conn, $sql);

        $sql_log = "SELECT * FROM log_employee WHERE e_id = '$e_id ' ORDER BY le_date_in DESC";
        $result_log = mysqli_query($conn, $sql_log);
        $row_log = mysqli_fetch_array($result_log);

        $_SESSION["log_id"] = $row_log['le_id'];
